...
| 参数 | 默认 | 说明 |
|---|---|---|
| `-Name <名字>` | `claude-dev` | VM 名(多 VM)。**交互终端一般不用敲**:start 弹菜单选/新建,stop/delete 多台时也弹;此参数留给脚本/非交互。每台 VM 状态独立子目录,bundle/SSH key 共享。status:交互多台弹菜单选台,非交互多台给总览 |
| `-Cpus <n>` | `4` | VM CPU 核数 |
...
.\scripts\launch.ps1 start -Name dev-java -MemoryGB 4  # 起第二台 VM(非交互写法;交互终端用菜单 "+ 新建 VM...")
```
...
真人终端裸跑 `start` 不用记 `-Name`:先弹**单选菜单**(↑↓ / 回车 / 数字键),选项是受管 VM 列表(标注 Running/Stopped)+ 末项 `+ 新建 VM...`;**回车 = 上次使用的台**(记录在状态目录 `.last-vm`)。

选 `+ 新建 VM...` 进向导,一条命令完成建机:

1. 输入 VM 名(小写字母/数字/连字符,如 `dev-java`)
2. 逐项输入挂载的宿主机目录(`D:\code\my-project` 或 `D:\code\my-project=别名`,回车结束,至少一条;目录不存在当场重输)——写入该 VM 子目录的 `mounts.txt`
3. 随后走正常 start(接着弹"可选特性"菜单现选)

`stop` / `delete` / `status` 在受管 VM 多于一台时弹同款菜单(status 的菜单列表带状态标签,本身就是总览;stop/delete 菜单无"新建"项);单台直接执行。非交互(管道/后台/Claude 代跑)永不弹菜单:start/stop/delete 直走 claude-dev 或显式 `-Name`,status 多台给总览。
