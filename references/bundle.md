...
bundle 存 cloud-init 离线安装用的本地包,**让 VM 创建不依赖网络下载**。位置在状态目录:默认 `%USERPROFILE%\.claude-dev-vm\bundle\`(参数 `-StateDir` / 环境变量 `CLAUDE_DEV_VM_HOME` 可改),不占用 skill 包目录,升级 skill 不受影响。
